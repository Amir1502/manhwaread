...
Обновление v0.1.3 (читалка и качество перевода): оверлей перевода рисуется
с маскирующей подложкой по форме бабла — BubbleHitArea расширена аддитивными
полями shape/polygon/fillColorArgb в chapter.json (старые архивы читаются
с дефолтами ELLIPSE/пусто/null), перевод не запекается в растр по-прежнему.
Читалка в полном экране: immersive-системные бары (WindowInsetsControllerCompat),
скрытый нижний таббар приложения на маршруте reader/*, автоскрытие панелей
читалки по тапу мимо бабла. Конвейер перевода: строки KO/JA-движков OCR без
единого CJK-символа отбрасываются до dedupe (фильтр dropCjkEngineJunk),
TypesettingCompositor пропускает только пригодные переводы (не эхо оригинала
и содержит кириллицу), декоратор SegmentPersistingTranslator пишет
translatedText в SegmentStore — карточка бабла показывает цельный текст
(fallback — строки OverlaySpec). FileChapterLoader толерантен к битому
overlays.json (runCatching → пустой слой).

Обновление v0.1.4 (стабильность читалки, стриминг, тайтлы, удаление загрузок):
тайловый рендеринг больше не «моргает черным»: незадекодированные тайлы
рисуются серым плейсхолдером, повторный идентичный запрос не отменяет
текущее декодирование (счётчик поколений растёт только при реальном изменении
набора тайлов), декодирование вебтун-полосы обрезается по видимой полосе
вьюпорта (getLocalVisibleRect), inSampleSize определяется абсолютным
масштабом — LRU-трешинг на страницах 800×20000 устранён. Ошибки больше не
молчаливы: битый файл страницы помечается ReaderPage.isCorrupted (глава
открывается, битая страница показывает плейсхолдер фиксированной высоты),
отказ декодера — TiledImageView.onDecodeError → оверлей с кнопкой «Повторить».
Чтение без скачивания: клик по главе всегда открывает читалку; если офлайн-
каталога нет, StreamingChapterLoader (:app) грузит страницы источника в
cacheDir/stream/chapter_{id} (OkHttp + Referer, валидация magic bytes,
переиспользование полного кэша), ReaderNavScreen показывает прогресс и retry;
стриминг — без перевода (пустые оверлеи), скачивание — явная кнопка в списке
глав. Перевод тайтла: MangaEntity.titleRu (БД v2, MIGRATION_1_2
«ALTER TABLE manga ADD COLUMN titleRu TEXT») + MangaDao.setTitleRu;
TitleTranslator (:core:translation-api, один сегмент, валидация «не эхо +
содержит кириллицу») запускается один раз фоном при открытии карточки, если
провайдер выбран; titleRu переживает refresh карточки (toEntityPreserving);
UI показывает titleRu ?: title (карточка, библиотека); MangaDex
TITLE_LANG_PRIORITY теперь ru-first. Удаление скачанных глав: ChapterDeleter
(:feature:downloads) удаляет каталог главы и строки
download_tasks/translation_jobs/segments (новые deleteForChapter(chapterId)
в DAO; строка chapters и история чтения сохраняются — главу можно скачать
заново); кнопка удаления с диалогом подтверждения на каждой карточке задачи
в «Загрузках». Целостность загрузок: запись страниц атомарная (tmp+rename),
байты валидируются как изображение перед сохранением и перед «пропустить уже
скачанное» (isDecodableImage: magic bytes + inJustDecodeBounds) — антибот-HTML
